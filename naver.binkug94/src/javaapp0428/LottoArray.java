...
public class LottoArray {
	public static void main(String [] args) {
		...
		/*
		 * for(int i=1;i<size;i++) { //현재 데이터를 임시변수에 복사 int sta = lotto[i]; //자신의 앞에 있는
		 * 모든 데이터와 비교 //뒤에서부터 앞으로 비교 int j = i-1; for(; j>=0;j=j-1) { //비교하는 데이터가 클때는
		 * //비교하는 데이터를 다음 자리로 이동 //비교하는 데이터가 작거나 같으면 반복문 중단 if(sta < lotto[j]) {
		 * lotto[j+1] = lotto[j]; }else { break; } } //앞쪽의 데이터와 비교가 끝나면 기존 데이터를 비교가 끝난
		 * 다음자리에 복사 lotto[j+1] = sta; }
		 */
